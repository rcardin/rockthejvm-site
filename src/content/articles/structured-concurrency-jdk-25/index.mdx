...
class GitHubRepository
  ...
  @Override
  public List<String> findAllFiles(Repository repository) {
    ...
    default -> throw new RuntimeException("Unknown repository '%s'".formatted(repository.name));
    }
  }
}
```

As we can see, only the `raise4s` repository has a `README.md` file. Now, we can implement the use case to find the README file from multiple repositories:
